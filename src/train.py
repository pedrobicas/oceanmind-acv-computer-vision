...
Uso recomendado:
1) python src/generate_synthetic_ocean_dataset.py --output dataset --images-per-class 220 --size 96
2) python src/train.py --dataset dataset --epochs 12
"""
...
def make_datasets(dataset_dir: str, image_size: int, batch_size: int, seed: int):
    ...
    # Divide validação temporária em validação e teste.
...
def evaluate_model(model, test_ds, class_names, model_name: str, reports_dir: Path):
    ...
    plot_confusion_matrix(cm, class_names, f"Matriz de Confusão - {model_name}", reports_dir / f"confusion_matrix_{model_name}.png")
...
def main():
    ...
    for name, builder in [("cnn_v1", build_cnn_v1), ("cnn_v2", build_cnn_v2)]:
        ...
        results[name] = {"test_accuracy": float(test_acc), "test_loss": float(test_loss), "classification_report": report}

    with open(reports_dir / "model_comparison.json", "w", encoding="utf-8") as f:
        json.dump({"classes": class_names, "results": results}, f, ensure_ascii=False, indent=2)
